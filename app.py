...
st.title("🏗️ Industrial Decarbonization AI Platform") 
...
if "scada_logger_db" not in st.session_state: 
    # Synchronized initial structural headers to avoid concat formatting errors
    st.session_state.scada_logger_db = pd.DataFrame(columns=[ 
        "Timestamp (IST)",
        "Live Throughput (T/h)",
        "Live Power Demand (kW)", 
        "Measured Emissions (kg CO2)", 
        "AI Predicted Baseline (kg)", 
        "Emissions Residual Gap (kg)", 
        "Carbon Tax ($/Ton)", 
        "Electricity Rate ($/kWh)", 
        "2s Cycle Savings (USD)", 
        "2s Cycle Savings (INR)" 
    ]) 
...
# ==================================================== 
# ⏱️ REAL-TIME 2-SECOND MATHEMATICS 
# ==================================================== 
...
# ==================================================== 
# 📈 FEATURE A: DYNAMIC REAL-TIME LINE CHART (WITHOUT DATE & TIME INDEX)
# ==================================================== 
st.markdown("### 📈 Real-Time Rolling Shift Trend Tracking") 
# Dropped index reference so the chart plots sequential live metrics without dates
chart_df = st.session_state.scada_logger_db[["Live Power Demand (kW)", "Measured Emissions (kg CO2)"]] 
st.line_chart(chart_df) 

# ==================================================== 
# 💾 FEATURE B: ROW-BASED COMPLIANCE EXPORT LEDGER (WITH DATE & TIME)
# ==================================================== 
st.markdown("### 💾 Regulatory Compliance & ESG Data Export (Horizontal Registry)") 
# This preserves both the Timestamp (IST) column and all tracking data rows
